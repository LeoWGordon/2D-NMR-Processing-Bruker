...
function NMR2DPlotter
    ...
    h.c(7) = uicontrol('Style','popupmenu','Units','normalized',...
                    'Position',[0.8,0.8,0.15,0.05],'String',{'<HTML><sup>1</sup>H','<HTML><sup>13</sup>C','<HTML><sup>27</sup>Al'}); %F2 dropdown
    ...
    h.c(9) = uicontrol('Style','popupmenu','Units','normalized',...
                    'Position',[0.8,0.725,0.15,0.05],'String',{'<HTML><sup>1</sup>H','<HTML><sup>13</sup>C','<HTML><sup>27</sup>Al'}); %F2 dropdown
    ...
    h.c(13) = uicontrol('Style','text','Units','normalized',...
                    'Position',[0.05,0.55,0.9,0.05],'String','Countour Threshold Factor','FontSize',14); %Threshold string
    h.c(14) = uicontrol('Style','text','Units','normalized',...
                    'Position',[0.05,0.45,0.9,0.05],'String','Number of Contour Levels','FontSize',14); %Levels nucleus string
    ...
    h.s(1) = uicontrol('Style','slider','Units','normalized',...
                   'Position',[0.05,0.525,0.9,0.05],'Min',0.1,'Value',0.1,'SliderStep',[0.005 0.1]);
    ...
    % Live update of threshold text box
               fun1 = @(~,e)set(h.c(11),'String',num2str(100*log10(10*get(e.AffectedObject,'Value'))));
    ...
    function p_load(varargin)
            ...
            guidata(gcf,DATA);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            function [shift, intensity] = rawNMRimp(filename)
...
%                 F1_nuc = 27;
        ...
        h.f(2) = figure;
        contour(xc,yc,thres,thresvec(:),'.-k','LineWidth',2);
        axis([F2ax F1ax])
        axis square
        axc=gca;
        ...
        try
            ...
            axy=axes('InnerPosition',conposy,'PositionConstraint','innerposition');
        ...
        axx=axes('InnerPosition',conposx,'PositionConstraint','innerposition');            
